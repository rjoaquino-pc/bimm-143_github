...
> (1946), the switch to the newer aP vaccine (1946), the switch to the
...
ggplot(cdc) +
  ...
  geom_point() +
  geom_line() +
  geom_vline(xintercept = 1946, col ="dodgerblue1", lty=2) +
  geom_vline(xintercept = 1996, col ="firebrick", lty=2) +
...
**Why is this vaccine-preventable disease on the upswing?** o answer
...
read this in T with the `read_json` function from the **jsonlite**
package:
...
> Asian females, White males etc…)? (In terms of race and gender is this
...
> over time? Q13. Complete the following code to make a summary boxplot
> of Ab titer levels (MFI) for all antigens:
...
## Focus on “PT” Pertussis Toxin antingen
...
ggplot(pt.igg.21) +
  ...
  geom_point() +
geom_line() +
geom_vline(xintercept =  14, col="gray", lty=2)
```
...
> Q. 14 Final plot including geom_smooth and span function
